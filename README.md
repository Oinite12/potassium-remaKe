...
- Jokers that can evolve with Substance (Glop-themed evolutions) can be implemented by adding a key-value pair to the table `Potassium.glop_evolutions`, where the key is the key of the initial Joker, and the value is the key of the evolved Joker.
...
- A flag can be added to blinds in a similar manner to showdown bosses that will mark them as "banana blinds". To do so, add the following key-value pair to your SMODS.Blind: `boss = {banana = true}`. Banana blinds are boss blinds that only appear on ante 12 and have their own special background; ante 12, and only ante 12, exclusively has banana blinds in its pool.
